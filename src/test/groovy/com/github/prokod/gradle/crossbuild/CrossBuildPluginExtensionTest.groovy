...
"""
        ...
        '7.6.4'       | '2.12'              | '-def_?' | null       | null       | null       | '-def_?'   | '-def_?'   | '-def_?'
        '8.7'         | '2.11'              | '-def_?' | '-1-6-0_?' | '-2-4-0_?' | '-2-4-1_?' | '-1-6-0_?' | '-2-4-0_?' | '-2-4-1_?'
    }
...
"""
        ...
        where:
        gradleVersion | defaultScalaVersion | tcs       | ap   | otcs1   | otcs2 | otcs3 | oap3 | etcs1     | etcs2     | etcs3     | eap3
        '7.6.4'       | '2.11'              | 'default' | '-def_?' | 'max' | 'fail' | null  | null       | 'max'   | 'fail'  | 'default' | '-def_?'
        '8.7'         | '2.12'              | 'default' | '-def_?' | 'max' | 'fail' | null  | '-2-4-1_?' | 'max'   | 'fail'  | 'default' | '-2-4-1_?'
    }
}